...
class AlbertPreTraining(nn.Cell):
    """
    ...
    def __init__(self, config, is_training, use_one_hot_embeddings):
        super(AlbertPreTraining, self).__init__()
...
class AlbertPretrainingLoss(nn.Cell):
    """
    ...
    def __init__(self, config):
        super(AlbertPretrainingLoss, self).__init__()
...
class AlbertNetworkWithLoss(nn.Cell):
    """
    ...
    def __init__(self, config, is_training, use_one_hot_embeddings=False):
        super(AlbertNetworkWithLoss, self).__init__()
        self.albert = AlbertPreTraining(config, is_training, use_one_hot_embeddings)
        self.loss = AlbertPretrainingLoss(config)
        self.cast = P.Cast()
    ...
    def construct(self,
        ...
        prediction_scores, seq_relationship_score = \
            self.albert(input_ids, input_mask, token_type_id, masked_lm_positions)
        total_loss = self.loss(prediction_scores, seq_relationship_score,
                               masked_lm_ids, masked_lm_weights, next_sentence_labels)
        return self.cast(total_loss, mstype.float32)


class AlbertTrainOneStepCell(nn.TrainOneStepCell):
    """
    ...
    def __init__(self, network, optimizer, sens=1.0, enable_clip_grad=True):
        super(AlbertTrainOneStepCell, self).__init__(network, optimizer, sens)
        self.cast = P.Cast()
...
class AlbertTrainOneStepWithLossScaleCell(nn.TrainOneStepWithLossScaleCell):
    """
    Encapsulation class of albert network training.
    ...
    def __init__(self, network, optimizer, scale_update_cell=None):
        super(AlbertTrainOneStepWithLossScaleCell, self).__init__(network, optimizer, scale_update_cell)
        self.cast = P.Cast()
        self.degree = 1
...
class AlbertTrainOneStepWithLossScaleCellForAdam(nn.TrainOneStepWithLossScaleCell):
    ...
    """
    def __init__(self, network, optimizer, scale_update_cell=None):
        super(AlbertTrainOneStepWithLossScaleCellForAdam, self).__init__(network, optimizer, scale_update_cell)
        self.cast = P.Cast()
        self.degree = 1
        if self.reducer_flag:
            self.degree = get_group_size()
...
class AlbertTrainAccumulationAllReducePostWithLossScaleCell(nn.Cell):
    """
    Encapsulation class of albert network training.
    ...
    def __init__(self, network, optimizer, scale_update_cell=None, accumulation_steps=1, enable_global_norm=False):
        super(AlbertTrainAccumulationAllReducePostWithLossScaleCell, self).__init__(auto_prefix=False)
...
class AlbertTrainAccumulationAllReduceEachWithLossScaleCell(nn.Cell):
    ...
    """
    def __init__(self, network, optimizer, scale_update_cell=None, accumulation_steps=1, enable_global_norm=False):
        super(AlbertTrainAccumulationAllReduceEachWithLossScaleCell, self).__init__(auto_prefix=False)
        self.network = network
        self.network.set_grad()
        self.weights = optimizer.parameters
        self.optimizer = optimizer
...
class AlbertPretrainEval(nn.Cell):
    '''
    Evaluate MaskedLM prediction scores
    '''
    def __init__(self, config, network=None):
        super(AlbertPretrainEval, self).__init__(auto_prefix=False)
        if network is None:
            self.network = AlbertPreTraining(config, False, False)
        else:
            self.network = network
