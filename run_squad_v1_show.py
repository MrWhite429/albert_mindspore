...
def do_train(dataset=None, network=None, load_checkpoint_path="", save_checkpoint_path="", epoch_num=1, args=None):
    ...
    print('run 025')
    model.train(epoch_num, dataset, callbacks=[LossCallBack(dataset.get_dataset_size()), ckpoint_cb], dataset_sink_mode=True)
    # model.train(epoch_num, dataset, callbacks=[TimeMonitor(dataset.get_dataset_size()), eval_callback,LossCallBack(dataset.get_dataset_size()), ckpoint_cb], dataset_sink_mode=True)
...
def run_squad():
    ...
    if args_opt.do_eval.lower() == "true":
        ...
        from src.squad_postprocess import SQuad_postprocess,SQuad_showpredict
        tokenizer = tokenization.FullTokenizer(vocab_file=args_opt.vocab_file_path,
                                               do_lower_case=True,
                                               spm_model_file=args_opt.spm_model_file)
        while True:
            if show_mode.lower() == "true":
                from src.squad_utils import read_squad_example_from_input
                para_txt = input('paragraph:')
                if para_txt=='exit':
                    break
                qa_txt = input('question:')
                
                # para_txt = 'Architecturally, the school has a Catholic character. Atop the Main Building\'s gold dome is a golden statue of the Virgin Mary. Immediately in front of the Main Building and facing it, is a copper statue of Christ with arms upraised with the legend "Venite Ad Me Omnes". Next to the Main Building is the Basilica of the Sacred Heart. Immediately behind the basilica is the Grotto, a Marian place of prayer and reflection. It is a replica of the grotto at Lourdes, France where the Virgin Mary reputedly appeared to Saint Bernadette Soubirous in 1858. At the end of the main drive (and in a direct line that connects through 3 statues and the Gold Dome), is a simple, modern stone statue of Mary.'
                # qa_txt = 'To whom did the Virgin Mary allegedly appear in 1858 in Lourdes France?'
                eval_examples = read_squad_example_from_input(para_txt,qa_txt)
            else:
            ...
            if not show_mode.lower() == "true":
                ...
                print(f'answer:{all_predictions[0]}')
